...
const rotaDays = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']

const rotaRows: { name: string; role: string; shifts: (string | null)[] }[] = [
  {
    name: 'Dilnoza A.',
    role: 'Floor',
...
export function RotaMock({ className }: { className?: string }) {
  return (
    <div className={cn('w-full rounded-2xl bg-card p-4 sm:p-5', className)}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold">Week 32 · Kitchen &amp; Floor</p>
            <p className="text-xs text-muted-foreground">Chorsu branch</p>
          </div>
        </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
        <p className="text-[11px] text-muted-foreground">Projected labour cost</p>
        <p className="text-[11px] font-semibold">
          14 820 000 <span className="text-muted-foreground">UZS</span>
        </p>
      </div>
    </div>
  )
...
export function ClockInMock({ className }: { className?: string }) {
  return (
    <div className={cn('w-full max-w-[260px] rounded-2xl bg-card p-5', className)}>
      ...
      <p className="font-display text-lg font-semibold">Anthony</p>

      <div className="mt-4 rounded-2xl bg-primary p-4 text-primary-foreground">
        <p className="text-[11px] text-primary-foreground/70">Today&apos;s shift</p>
        <p className="mt-0.5 text-sm font-semibold">12:00 – 20:00 · Floor</p>
        <div className="mt-4 flex items-center gap-2 rounded-full bg-primary-foreground/15 p-1.5">
          ...
          <span className="text-xs font-medium">Swipe to clock in</span>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {[
          ...
          { icon: Nfc, label: 'NFC' },
          { icon: CheckCircle2, label: 'On time' },
        ].map((chip) => (
          <span
            key={chip.label}
...
export function LiveAttendanceMock({ className }: { className?: string }) {
  const rows = [
    { name: 'Bekzod T.', dept: 'Kitchen', state: 'in', time: '08:02' },
    { name: 'Kamola S.', dept: 'Hosts', state: 'break', time: '13:40' },
    { name: 'Aziz R.', dept: 'Bar', state: 'missing', time: 'Expected 12:00' },
  ]
  return (
    <div className={cn('w-full rounded-2xl bg-card p-5', className)}>
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold">Who&apos;s in right now</p>
...
export function TimesheetMock({ className }: { className?: string }) {
  const rows = [
    { name: 'Dilnoza A.', hours: '38h 20m', pay: '3 250 000' },
    { name: 'Bekzod T.', hours: '41h 05m', pay: '3 780 000' },
    { name: 'Kamola S.', hours: '32h 45m', pay: '2 610 000' },
  ]
  return (
    <div className={cn('w-full rounded-2xl bg-card p-5', className)}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold">Timesheet · week 31</p>
          <p className="text-[11px] text-muted-foreground">Approved &amp; locked</p>
        </div>
        <span className="rounded-full bg-sage/10 px-2.5 py-1 text-[11px] font-medium text-sage">
          Payroll ready
        </span>
      ...
      <table className="mt-4 w-full text-left">
        <thead>
          <tr className="text-[10px] tracking-wide text-muted-foreground uppercase">
            ...
            <th className="pb-2 text-right font-medium">Pay (UZS)</th>
          </tr>
        </thead>
...
export function DemandCalendarMock({ className }: { className?: string }) {
  return (
    <div className={cn('w-full rounded-2xl bg-card p-5', className)}>
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold">Demand forecast · August</p>
        <div className="flex items-center gap-1.5">
          ...
          <span className="ml-1 text-[10px] text-muted-foreground">L1–L4</span>
        </div>
      </div>
...
export function RankMock({ className }: { className?: string }) {
  return (
    <div className={cn('w-full max-w-[280px] rounded-2xl bg-card p-5', className)}>
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-sm font-semibold">Tajribali</p>
          <p className="text-[10px] text-muted-foreground">Darajam · level 4</p>
        </div>
      </div>
      <div className="mt-4">
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          <span>6 420 points</span>
          <span>10 000</span>
        </div>
...
export function DonoOpaMock({ className }: { className?: string }) {
  return (
    <div className={cn('w-full rounded-2xl bg-card p-5', className)}>
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold">Dono opa</p>
      </div>
      <div className="mt-4 flex flex-col gap-2">
        <p className="ml-auto max-w-[80%] rounded-2xl rounded-br-md bg-primary px-3 py-2 text-[11px] leading-relaxed text-primary-foreground">
          Lag&apos;mon ichida yeryong&apos;oq bormi?
        </p>
        <p className="max-w-[85%] rounded-2xl rounded-bl-md bg-muted px-3 py-2 text-[11px] leading-relaxed text-secondary-foreground">
          Yo&apos;q — lag&apos;monda yeryong&apos;oq yo&apos;q. Ammo sous
          tarkibida kunjut bor, allergiya haqida mehmonni ogohlantiring.
        </p>
      </div>
    </div>
  )
...
export function ChecklistMock({ className }: { className?: string }) {
  const items = [
    { label: 'Fridge temperatures logged', done: true, photo: false },
    { label: 'Prep area photo', done: true, photo: true },
    { label: 'Till float counted', done: false, photo: false },
  ]
  return (
    <div className={cn('w-full rounded-2xl bg-card p-5', className)}>
      <p className="text-sm font-semibold">Opening checklist</p>
      <p className="text-[11px] text-muted-foreground">Kitchen · 2 of 3 done</p>
